...
and extracts all three MLP weight matrices for Layer 0:

    gate_proj  (M=6912, K=2560)  — gate arm of SwiGLU
    up_proj    (M=6912, K=2560)  — up arm of SwiGLU
    down_proj  (M=2560, K=6912)  — projection back to hidden dim
...
Output files:
  bitnet_layer_0_gate_proj.bin
  bitnet_layer_0_up_proj.bin
  bitnet_layer_0_down_proj.bin
...
# ═══════════════════════════════════════════════════════════════
# Packing / unpacking helpers  (identical to extract_weights.py)
# ═══════════════════════════════════════════════════════════════
...
# Layer 0 MLP tensors to extract
LAYER_0_MLP_TENSORS = [
    {
        "tensor_name": "model.layers.0.mlp.gate_proj.weight",
        "out_file":    "bitnet_layer_0_gate_proj.bin",
        "M": 6912,
        "K": 2560,
        "desc": "gate_proj (SwiGLU gate arm)",
    },
    {
        "tensor_name": "model.layers.0.mlp.up_proj.weight",
        "out_file":    "bitnet_layer_0_up_proj.bin",
        "M": 6912,
        "K": 2560,
        "desc": "up_proj (SwiGLU up arm)",
    },
    {
        ...
        "M": 2560,
        "K": 6912,
        "desc": "down_proj (projection back to hidden dim)",
    },
]
...
def extract_and_pack(tensors: dict, spec: dict, index: int, total: int):
    ...
    # Validate dimensions
    expected_packed_rows = M // 4
    assert packed_np.shape[0] == expected_packed_rows, \
        f"Expected {expected_packed_rows} packed rows, got {packed_np.shape[0]}"
    assert packed_np.shape[1] == K, \
        f"Expected K={K} columns, got {packed_np.shape[1]}"
...
def main():
    print("=" * 60)
    print("BitNet Full MLP Weight Extraction (Layer 0)")
    ...
    # ── 3. Extract each MLP weight ─────────────────────────────
    print(f"\n[Step 3] Extracting & packing {len(LAYER_0_MLP_TENSORS)} weight matrices ...")

    results = []
    for idx, spec in enumerate(LAYER_0_MLP_TENSORS, start=1):
        info = extract_and_pack(tensors, spec, idx, len(LAYER_0_MLP_TENSORS))
        results.append(info)

    # ── Summary ────────────────────────────────────────────────
    print("\n" + "=" * 60)
    print("DONE — Summary for JS kernel integration:")
    print("=" * 60)
    total_bytes = 0
    for r in results:
        print(f"\n  {r['name']}:")
    ...
    print(f"\n  Total weight data: {total_bytes:,} bytes ({total_bytes/1024:.1f} KB)")
    print("=" * 60)
